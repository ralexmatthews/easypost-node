...
describe("Batch Service", function () {
  ...
  it("retrieves all batches", async function () {
    const batches = await this.client.Batch.all({
  ...
  it("creates a scanform for a batch", async function () {
    ...
    const batchWithScanForm = await this.client.Batch.createScanForm(
      boughtBatch.id
    );
  ...
  it("adds and removes shipments from a batch", async function () {
    ...
    const addShipmentsResponse = await this.client.Batch.addShipments(
      batch.id,
      [shipment.id]
    );
    expect(addShipmentsResponse.num_shipments).to.equal(1);

    const removeShipmentsResponse = await this.client.Batch.removeShipments(
      batch.id,
      [shipment.id]
    );
  ...
  it("generates a label for a batch", async function () {
    ...
    const batchWithLabel = await this.client.Batch.generateLabel(
      boughtBatch.id,
      "ZPL"
    );
