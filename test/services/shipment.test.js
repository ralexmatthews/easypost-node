...
describe("Shipment Service", function () {
  ...
  it("retrieves next page of shipments", async function () {
    try {
      ...
      const nextPage = await this.client.Shipment.getNextPage(
        shipments,
        Fixture.pageSize()
      );
  ...
  it("gets the lowest smartrate from a list of smartRates", async function () {
    ...
    // Test lowest smartrate with valid filters
    const lowestSmartRate = this.client.Utils.getLowestSmartRate(
      smartRates,
      3,
      "percentile_90"
    );
  ...
  it("buys a shipment with end_shipper_id", async function () {
    const endShipper = await this.client.EndShipper.create(
      Fixture.caAddress1()
    );
  ...
  it("retrieve time-in-transit data for each of the Rates of a shipment", async function () {
    ...
    const estimatedDeliveryDates =
      await this.client.Shipment.retrieveEstimatedDeliveryDate(
        shipment.id,
        Fixture.plannedShipDate()
      );

    expect(
      estimatedDeliveryDates.every(
        (entry) => entry.easypost_time_in_transit_data
      )
    ).to.not.be.null;
  });
});
