...
describe("CustomsItem Service", function () {
  ...
  it("creates a customs item", async function () {
    const customsItem = await this.client.CustomsItem.create(
      Fixture.basicCustomsItem()
    );
  ...
  it("retrieves a customs item", async function () {
    ...
    const retrievedCustomsInfo = await this.client.CustomsItem.retrieve(
      customsItem.id
    );

    expect(customsItem.object).to.be.equal("CustomsItem");
    expect(withoutParams(retrievedCustomsInfo)).to.deep.include(
      withoutParams(customsItem)
    );
  });
});
