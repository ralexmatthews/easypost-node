...
describe("ReferralCustomer Service", function () {
  ...
  before(function () {
    const partnerUserProdApiKey =
      process.env.PARTNER_USER_PROD_API_KEY || "123";
  ...
  it("retrieves all referral users", async function () {
    const referrals = await this.client.ReferralCustomer.all({
  ...
  it("updates a referral user", async function () {
    ...
    await this.client.ReferralCustomer.updateEmail(
      singleReferral.id,
      testEmail
    ).then(
