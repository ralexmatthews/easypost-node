...
describe("Address Service", function () {
  ...
  it("retrieves an address", async function () {
    ...
    expect(withoutParams(retrievedAddress)).to.deep.include(
      withoutParams(address)
    );
  });

  it("retrieves all addresses", async function () {
    const addresses = await this.client.Address.all({
  ...
  it("retrieves next page of addresses", async function () {
    try {
      ...
      const nextPage = await this.client.Address.getNextPage(
        addresses,
        Fixture.pageSize()
      );
