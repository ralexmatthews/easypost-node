...
describe("Pickup Service", function () {
  ...
  it("retrieves a pickup", async function () {
    ...
    expect(withoutParams(retrievedPickup)).to.deep.include(
      withoutParams(pickup)
    );
  });

  it("retrieves all pickup", async function () {
    const pickups = await this.client.Pickup.all({
  ...
  it("retrieves next page of pickups", async function () {
    try {
      ...
      const nextPage = await this.client.Pickup.getNextPage(
        pickups,
        Fixture.pageSize()
      );
