...
describe("Insurance Service", function () {
  ...
  it("retrieves an insurance object", async function () {
    ...
    const retrievedInsurance = await this.client.Insurance.retrieve(
      insurance.id
    );
  ...
  it("retrieves next page of insurances", async function () {
    try {
      const insurances = await this.client.Insurance.all({
        page_size: Fixture.pageSize(),
      });
      const nextPage = await this.client.Insurance.getNextPage(
        insurances,
        Fixture.pageSize()
      );
