...
describe("BetaReferralCustomerService", function () {
  ...
  before(function () {
    this.referralCustomerProdApiKey =
      process.env.REFERRAL_CUSTOMER_PROD_API_KEY || "123";
  ...
  it("add payment method to a referral customer account", async function () {
    await this.client.BetaReferralCustomer.addPaymentMethod(
      "cus_123",
      "ba_123"
    ).catch((error) => {
  ...
  it("Refund by amount for a recent payment", async function () {
    await this.client.BetaReferralCustomer.refundByAmount(2000).catch(
      (error) => {
        expect(error.statusCode).to.equal(422);
  ...
  it("Refund a payment by a payment log ID", async function () {
    await this.client.BetaReferralCustomer.refundByPaymentLog(
      "paylog_..."
    ).catch((error) => {
      expect(error.statusCode).to.equal(422);
